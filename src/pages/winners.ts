...
import { IPagination, IController, IWinner, IAutos, IWinnersData } from 'src/types';
...
class WinnersPage {
  ...
  public async draw(page?: number): Promise<void> {
    ...
    const winners: IWinnersData | undefined = await this.controller.getWinners(this.page, this.limitWinners, this.sort, this.direction);
    const titleEl: HTMLElement = this.createTitle(winners?.countWinners);
    const paginationEl: HTMLElement | undefined = this.createPagination(winners);
    const winnersTable: HTMLTableElement = await this.createWinnersTable(winners);
    if (paginationEl) {
  ...
  private createPagination(winners: IWinnersData | undefined): HTMLElement | undefined {
    if (winners) {
      const paginationEl: HTMLElement = this.pagination.createPagination(this.getPage.bind(this), this.getLimitWinners.bind(this), winners.countWinners, this.draw.bind(this));
      return paginationEl;
    }
  }

  private async createWinnersTable(winners: IWinnersData | undefined): Promise<HTMLTableElement> {
    ...
    if (winners && carsAll) {
      winners.winners.forEach((winner: IWinner, index: number) => {
        const car: IAutos | undefined = carsAll.find((car) => car.id === winner.id);
        if (car) {
          const rowsBlock: HTMLElement = this.createRowsBlock((this.page - 1) * this.limitWinners + (index + 1), car.name, winner.wins, winner.time, car.color);
  ...
  private createRowsBlock(number: number, name: string, wins: number, time: number, color: string): HTMLElement {
    const rowsBlock: HTMLElement = document.createElement('tr');
    rowsBlock.classList.add('winners__rows');
    const numberEl: HTMLElement = this.createRow(number.toString(), 'winners__number');
    ...
    const winsEl: HTMLElement = this.createRow(wins.toString(), 'winners__wins');
    const timeEl: HTMLElement = this.createRow(time.toString(), 'winners__time');
